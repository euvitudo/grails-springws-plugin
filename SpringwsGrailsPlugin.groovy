...
class SpringwsGrailsPlugin {
    ...
    def author = "Russ Miles" // and Phillip Warner

    def authorEmail = "russ@russmiles.com" // and euvitudo@gmail.com
...
* New in 0.7.0: Updated to Grails 2.3.5;  converted tests to spock functional tests
...
* When consuming services, a WebServiceTemplate is provided, in much the same vein as the Spring WebServiceTemplate, that integrates more naturally with the rest of your Groovy code.'''
    ...
    def artefacts = [
        ...
        InterceptorsConfigArtefactHandler,
        WsSecurityConfigArtefactHandler
    ]
    ...
    static final INTERCEPTOR_BEANS = { interceptor ->
        "${interceptor.fullName}Class"(MethodInvokingFactoryBean) {
            ...
            arguments = [
                InterceptorsConfigArtefactHandler.TYPE,
                interceptor.fullName
            ]
        }
    ...
    def doWithSpring = {
        ...
        for(endpointClass in application.getArtefacts(EndpointArtefactHandler.TYPE)) {
            ...
            if(wsdlConfig){
                ...
                def beanName = "${wsdlConfig.wsdlName ?: name}"
                if (wsdlConfig.file) {
                    "$beanName"(SimpleWsdl11Definition, wsdlConfig.file)
                } else {
                    "$beanName"(DefaultWsdl11Definition){
                        schemaCollection = {CommonsXsdSchemaCollection s->
                            inline = wsdlConfig.expandImports ?: false
        ...
        if (foundAcegi) {
            ...
            wsSecurityAccessDecisionManager(SpringwsGrailsPlugin.classLoader.loadClass('org.springframework.security.vote.AffirmativeBased')) { decisionVoters = [wsSecurityRoleVoter]}
